...
def test_near_perfect_win_rate_over_a_small_sample_is_forgiven() -> None:
    # Vetted wallet #10 ("dead") runs 95.6% over 45 exits; small perfect
    # streaks happen to patient humans.
...
def test_extreme_exit_frequency_is_a_bot() -> None:
    # The 881-exits-in-2-days exclusion; the busiest vetted human does ~25/day.
...
def test_large_month_pnl_with_almost_no_trades_is_a_static_holder() -> None:
    # The $13M/month whale whose PnL came from holdings, not trading.
    reason = classify_bot(
        metrics(trade_count=3, win_rate="1", realized_pnl="1200"),
...
def test_low_activity_with_modest_pnl_is_just_a_quiet_trader() -> None:
    assert classify_bot(metrics(trade_count=3, win_rate="1"), month_pnl=Decimal("9000")) is None


def test_static_holdings_check_needs_coarse_data() -> None:
    assert classify_bot(metrics(trade_count=3, win_rate="1"), month_pnl=None) is None
