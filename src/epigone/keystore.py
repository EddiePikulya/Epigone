...
class AgentKeystore:
    """Agent keys at rest, by user-account id. At most one active key per user
    (a rotation revokes before it stores); revoked rows stay as tombstones so
    an agent address can never be reused after deregistration (nonce-replay
    hazard, ADR-0005)."""
        ...
        *,
        ...
        try:
            await self._pool.execute(
                """
                INSERT INTO agent_keys (user_id, master_address, agent_address, agent_name,
                                        kek_id, dek_wrapped, key_ciphertext, created_at,
                                        expires_at)
            ...
            if error.constraint_name == "agent_keys_one_active_per_user":
                raise KeystoreError(
                    f"user {user_id} already has an active agent key — revoke it first"
                ) from None
            raise KeystoreError(
    ...
    async def signer(self, user_id: int) -> LocalAccount:
        """The gateway seam (issue #133): decrypt the user's active agent key
        in-process, at signing time, into the LocalAccount the SDK's signing
        helpers take. Refuses expired keys — Hyperliquid would reject their
        signatures anyway, and refusing here turns that into a clear error."""
        row = await self._active_row(user_id)
        if row is None:
            raise KeystoreError(f"no active agent key for user {user_id}")
        if row["expires_at"] <= self._clock.now():
            raise KeystoreError(
                f"agent key for user {user_id} expired at {row['expires_at']:%Y-%m-%d} — "
                f"rotate it (docs/runbooks/agent-key-rotation.md)"
            )
    ...
    async def revoke(self, user_id: int) -> AgentKeyRecord:
        """Tombstone the user's active key. The row stays forever: the UNIQUE
        agent_address is what enforces never-reuse-after-deregistration."""
        row = await self._pool.fetchrow(
            """
            UPDATE agent_keys SET revoked_at = $2
            WHERE user_id = $1 AND revoked_at IS NULL
            RETURNING user_id, master_address, agent_address, agent_name, kek_id,
    ...
    async def active_record(self, user_id: int) -> AgentKeyRecord | None:
        row = await self._active_row(user_id)
    ...
    async def list_records(self) -> list[AgentKeyRecord]:
        rows = await self._pool.fetch(
            """
            SELECT user_id, master_address, agent_address, agent_name, kek_id,
                   created_at, expires_at, revoked_at
    ...
    async def _active_row(self, user_id: int) -> asyncpg.Record | None:
        return await self._pool.fetchrow(
            """
            SELECT user_id, master_address, agent_address, agent_name, kek_id,
                   dek_wrapped, key_ciphertext, created_at, expires_at, revoked_at
            FROM agent_keys WHERE user_id = $1 AND revoked_at IS NULL
            """,
            user_id,
        )
...
def _print_record(record: AgentKeyRecord) -> None:
    ...
    print(
        ...
        f"  name {name}  {status}  expires {record.expires_at:%Y-%m-%d}  "
        f"kek {record.kek_id}"
    )
...
async def _cli(argv: list[str]) -> None:
    ...
    try:
        if args.command == "generate":
            ...
            record = await keystore.revoke(args.user)
