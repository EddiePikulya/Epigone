...
- **The 2000 messages/min allowance is OUTBOUND-ONLY.** One connection was driven to **6702 inbound messages/min sustained over 200s (peak minute 7497)** — 3.3–3.7× the allowance — while sending nothing but a keepalive ping every 25s. Zero error frames, zero throttling, never disconnected. So the Trader ceiling is set by the **subscription** cap, not by message volume: at 2 subscriptions per Trader against 1000, that is **499 Traders on one connection**, and the 10-connection cap is headroom rather than the binding constraint.
